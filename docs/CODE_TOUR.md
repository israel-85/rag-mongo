# rag-mongo: Ingestion Pipeline Walkthrough

Onboarding tour of the PDF → tag → chunk → embed → MongoDB Atlas Vector Search pipeline.
**Ref:** `main`

Scope: ingestion only (`load_data.py`). The retrieval half (`rag.py`) is not covered here — see
the "Retrieval architecture" section of `CLAUDE.md`.

## Step 1 — Start Here
`CLAUDE.md:1`

Situation: this is the whole repo's map in one file. Mechanism: it documents setup, the 5-stage pipeline, and config shape. Implication: read this before touching load_data.py, it tells you what each function is for. Gotcha: it's the source of truth, not the code comments - if they disagree, trust this file first, then verify against code.
...
Situation: raw PDF pages come in via PyPDFLoader, many are front matter or noise. Mechanism: filter_pages drops any page with 20 or fewer words. Implication: this is the cheapest filter in the pipeline - runs before the expensive LLM tagging step. Gotcha: threshold is a strict > 20, not >=, so a 20-word page is dropped (see the test at line 7 of test_load_data.py).
...
## Step 10 — How the Pure Functions Are Tested
`tests/test_load_data.py:1`

Situation: every top-level function in load_data.py has direct unit tests here. Mechanism: tests import straight from load_data (line 3) - safe because module import triggers no I/O. Fixtures (make_page, mock_tagger) live in tests/conftest.py and fake out Document creation and the LLM tagger. Implication: this is your template for testing any new pure function you add. Gotcha: conftest.py stubs sys.modules["key_param"] before import (line 9) - without that, importing load_data would need a real key_param.py.

## Step 11 — Next Steps

You've walked the full pipeline: filter → tag → merge → chunk → batch → embed → upsert, plus how it's tested. To run it for real: `cp key_param.example.py key_param.py`, fill in secrets, start your LM Studio server, then `python load_data.py`. To extend it, add a pure function near the top of load_data.py and a matching test in tests/test_load_data.py first.
