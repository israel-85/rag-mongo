...
Situation: this is the whole repo's map in one file. Mechanism: it documents setup, the 5-stage pipeline, and config shape. Implication: read this before touching loda_data.py, it tells you what each function is for. Gotcha: it's the source of truth, not the code comments - if they disagree, trust this file first, then verify against code.
...
`tests/test_loda_data.py:1`

Situation: every top-level function in loda_data.py has direct unit tests here. Mechanism: tests import straight from loda_data (line 3) - safe because module import triggers no I/O. Fixtures (make_page, mock_tagger) live in tests/conftest.py and fake out Document creation and the LLM tagger. Implication: this is your template for testing any new pure function you add. Gotcha: conftest.py stubs sys.modules["key_param"] before import (line 9) - without that, importing loda_data would need a real key_param.py.

## Step 11 — Next Steps

You've walked the full pipeline: filter → tag → merge → chunk → batch → embed → upsert, plus how it's tested. To run it for real: `cp key_param.example.py key_param.py`, fill in secrets, start your LM Studio server, then `python loda_data.py`. To extend it, add a pure function near the top of loda_data.py and a matching test in tests/test_loda_data.py first.
