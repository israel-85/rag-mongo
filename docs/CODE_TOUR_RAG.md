...
Situation: 84-line script, same convention as load_data.py. Mechanism: pure functions at module top level (`resolve_query`, `make_embeddings`, `stream_answer`); all I/O — MongoClient, Atlas, the LLM — confined to `main()` (line 41+), guarded by `if __name__ == "__main__"`. Implication: importing this module (as tests do) opens no MongoClient and pulls no PDF/ingest deps into the query path. Gotcha: don't move I/O out of main() or you break the two structural test guards that assert exactly this.

## Step 3 — Resolving the Query
`rag.py:27`

Situation: `resolve_query(argv)` decides what question gets asked. Mechanism: first CLI arg if present, else `DEFAULT_QUERY`. Implication: `python rag.py "how does sharding work?"` and `python rag.py` both just work, no flag parsing needed. Gotcha: no validation on the CLI arg — an empty string `argv[1]` would pass straight through as the query.
